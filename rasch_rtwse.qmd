...
kf1 <- RIinfitKfold(d,k = 20, sim_iter = 100, sim_cpu = cores)
RIinfitKfoldPlot(kf1)
```
### Item-restscore
```{r}
RIrestscore(d)
RIbootRestscore(d, samplesize = nrow(d),cpu = 8)
```
### PCA
```{r}
simpca <- RIbootPCA(d, cpu = 8,iterations = 300)
simpca$max
RIpcmPCA(d)
```
### Residual correlations
```{r}
simcor1 <- RIgetResidCor(d, 1000, 8)
RIresidcorr(d, cutoff = simcor1$p995)
```
...
#RIitemCats(d, items = c("q1","q3","q8","q10"))
```
...
kf2 <- RIinfitKfold(d,k = 20, sim_iter = 100, sim_cpu = cores)
RIinfitKfoldPlot(kf2)
```
### Item-restscore
```{r}
RIrestscore(d)
...
### Residual correlations
```{r}
simcor2 <- RIgetResidCor(d, 1000, 8)
...
Item 10 ("I can motivate myself to perform my job") is high in item infit (underfit), although item-restscore indicates no problems. It works well in terms of item-restscore, but item fit has better properties in detecting misfit in small samples. Item 10 also deviates in the loadings plot.
...
RIitemfit(d, simfit3)
kf3 <- RIinfitKfold(d,k = 20, sim_iter = 100, sim_cpu = cores)
RIinfitKfoldPlot(kf3)
```
### Item-restscore
```{r}
RIrestscore(d)
...
No misfit. While we still have small distances between item threshold locations t2 and t3 for items 1, 4, and 8, none are disordered.
